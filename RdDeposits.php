...
$history = new AccountData();
 $rd_history = $history->getRecurringDeposits($userId, $accountType);
...
<body>
    <?php if (isset($rd_history['error'])): ?>
        <p><?php echo $rd_history['error']; ?></p>
    <?php else: ?>
        <h2>Recurring Deposit History</h2>
        <table>
            <tr>
                <th>User ID</th>
            ...
            <?php foreach ($rd_history as $rd): ?>
                <tr>
                    <td><?= $rd['user_id']; ?></td>
                    <td><?= $rd['monthly_amount']; ?></td>
                    <td><?= $rd['interest_rate']; ?></td>
                    <td><?= $rd['intrest_amount']; ?></td>
                    <td><?= $rd['maturity_amount']; ?></td>
                    <td><?= $rd['duration_in_months']; ?></td>
                    <td><?= $rd['principal_amount']; ?></td>
                    <td><?= $rd['created_at']; ?></td>
                </tr>
            <?php endforeach; ?>
        </table>
